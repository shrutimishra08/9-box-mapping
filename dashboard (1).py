...
try:
    ...
    if not all(col in data.columns for col in required_cols):
        st.error("Dataset is missing required columns. Please check employee_data.csv.")
        st.stop()
...
try:
    ...
    employees_df['Best_Match'], _ = zip(*employees_df.apply(lambda row: match_candidate(row['Diplomatic'], row['Balanced'], 
                                                                                    row['Sociable'], row['Innovative'], default_weights), axis=1))
