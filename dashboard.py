...
# Succession pipeline candidates (top 10% based on potential alignment)
def get_succession_pipeline(data, personas):
    data['Potential_Score'] = data.apply(lambda row: min([np.sqrt(sum((np.array([row['Diplomatic'], row['Balanced'], row['Sociable'], row['Innovative']]) - 
                                                                    np.array([personas[dept][m] for m in ['Diplomatic', 'Balanced', 'Sociable', 'Innovative']])) ** 2)) 
                                                         for dept in personas], default=0), axis=1)
    top_10 = data.nlargest(10, 'Potential_Score')
...
employees_df['Best_Match'], _ = match_candidate(employees_df['Diplomatic'], employees_df['Balanced'], 
                                              employees_df['Sociable'], employees_df['Innovative'], default_weights)
employees_df['Performance'] = employees_df[performance_col]
employees_df['Potential'] = 100 - employees_df.apply(lambda row: min([np.sqrt(sum((np.array([row['Diplomatic'], row['Balanced'], 
...
# Interactive candidate matching (optional)
